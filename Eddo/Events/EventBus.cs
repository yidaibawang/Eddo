...
using Eddo.Events.EventStore;
using Eddo.Events.Handler;
using System;
...
namespace Eddo.Events
{
    ...
    public class EventBus : IEventBus
    {
        ...
        public void Trigger(Type eventType, object eventSource, IEventData eventData)
        {
            ...
            var handlers = _eventStore.GetHandlersForEvent(eventType);
            if (handlers.Any(th => th == eventType))
            {
                var handlerInterface = eventType.GetInterface("IEventHandler`1");

                var eventHandlers = IocContainer.ResolveAll(handlerInterface);
                //循环遍历，仅当解析的实例类型与映射字典中事件处理类型一致时，才触发事件
                foreach (var eventHandler in eventHandlers)
                {    
                    if (eventHandler.GetType() == eventType)
                    {   
                        var method = eventHandler.GetType().GetMethod(
                         "HandleEvent",
                         new[] { eventType }
                     );
        ...
        {
            return Task.Run(() => Trigger(eventType, eventSource, eventData));
        }
        #endregion
    }
}
